...
每个市场分别维护 5、15、30 日标签，不跨周期复用最终权重。第一阶段只有 5 日模型可以申请晋级；15/30 日继续积累严格样本外证据，达到门槛后再开放训练与晋级。
...
公司行动覆盖率区分“发生了公司行动”和“历史区间已被可靠查询”。ASX 使用官方逐年公告，A 股可使用 BaoStock 历史复权因子，美股使用 FMP/EODHD 历史分红与拆股接口。区间查询回执只参与数据完整性门控，不进入事件模型，避免把“无事件”错误编码成利好或利空。
...
## 15. 2026-08-03 Runtime V3 训练闭环

Runtime V3 针对 OOF 阶段集中超时和无法恢复的问题做了以下调整：

- 完整特征矩阵按真实行情与标签内容哈希保存为 gzip 缓存，同一数据版本不重复构建。
- 每个 Purged Walk-Forward 折完成后立即保存 manifest、OOF 行与折指标；服务中断后从已完成折继续。
- OHLCV 派生数组与 MACD 全序列每只股票只计算一次，移除逐切片重复计算形成的平方级开销。
- 基线模型最多使用 50,000 条训练行，正式 OOF 测试行全部保留；大折使用有正则化的增量 Logistic，避免内存峰值。
- 第一阶段默认只训练 5 日 Bayesian Ridge / ElasticNet Logistic 基线；树模型保留为显式 Challenger，不再阻断基线闭环。
- 旧的逐票校准默认退出生产任务；只有显式设置 `HISTORICAL_BACKTEST_LEGACY_CALIBRATION_ENABLED=true` 才作为 Research 对照运行，且不得覆盖严格 OOF Champion。
- 完整历史面板批量写入 Parquet；后续训练优先复用最近 14 天内且不少于 750 根的本地历史，只向行情源补齐缺口。
- 报告按 Runtime 版本分开统计，旧任务失败不会被删除或包装成新版成功率。

当时的 ASX Runtime V3 训练完成了 208,436 条训练行、97 只股票、55,600 条 OOF、5 个滚动折。结果只有 3/5 折为正且总 Brier Skill 略低于 0，因此保持 Research。完成训练和通过生产验收是两个独立条件。

因子模型还增加了市场级闸门：即使单股票因子实验通过局部 OOS 条件，只要所属市场的严格 5 日 OOF 模型未达到 Production，因子对线上预测的权重仍为 0。
- 全模型报告同时生成 JSON、HTML 和 Word，证据不足项显示 unavailable；报告与训练晋级只使用可复现的固定样本外门槛，不依赖外部 AI 额度。
...
- 该次历史审计时 ASX 市场级注册表仍只有 1,627 行、10 只股票、68 个日期，US/CN 尚无市场级注册模型；此数字仅保留用于解释 Runtime V3 的整改起点，不再代表当前训练状态。

## 16. 2026-08-12 Runtime V4 三市场严格 OOF

Runtime V4 已经在 ASX、US 和 A 股完成大样本、按日期隔离的严格 OOF。树模型在依赖可用的折中真实运行，报告分别记录面板行、实际抽样行、拟合行和测试行，不再把全量面板行伪装成实际拟合行。所有标签使用下一交易日入场，并通过 purge/embargo 隔离重叠未来窗口。

| 市场 | 面板行 / 股票 / 日期 | Meta 测试 | 方向准确率 | Brier Skill | ECE | 真正 long-only Top10 方向 | Top10 成本后均值 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ASX | 726,239 / 343 / 2,471 | 101,805 / 303 日 | 60.23% | +0.0171 | 2.02% | 46.86% | +0.153% | Research / No Trade |
| US | 883,486 / 444 / 2,453 | 130,837 / 303 日 | 52.90% | +0.0051 | 1.26% | 51.51% | +0.215% | Research / No Trade |
| CN | 1,011,423 / 547 / 2,869 | 163,795 / 303 日 | 51.57% | -0.0025 | 1.89% | 48.24% | +0.231% | Research / No Trade |

这里的“真正 long-only Top10”只统计模型实际愿意做多的最高排序样本，不等同于双向选择后看起来较高的 selective-confidence 指标。三市场均未达到 57% Top10 命中率与置信区间门槛，因此没有模型晋升，也不会提高页面交易置信率。

当前数据边界：

- US 公司行动补齐已对 450 只股票运行，190 只获得真实记录或可信的零事件覆盖回执；其余请求因供应商额度耗尽保留为失败，不伪造覆盖。
- EODHD、Tiingo 和 Twelve Data 使用主密钥优先、备用密钥只在额度/权限/限流错误时轮转。备用池不能把供应商总套餐额度变成无限额度。
- ASX 官方公告时间可用于事件 PIT；ASIC 的数字化财务报告/XBRL 为自愿提交，免费公开源不能诚实提供完整的历史结构化数值基本面。因此缺口必须显示为未验证，不能用今天的财务数值回填过去。
- 每日流程只追加新完成 K 线并解析成熟标签；每周有足够新增独立日期才训练 Challenger。没有新证据时只评估，不重复拟合制造“进步”。

Runtime V4 的永久训练报告：

- `reports/model-training/model-training-report-20260811T130617-0cad8ca3.docx`（ASX）
- `reports/model-training/model-training-report-20260811T134335-d1ddaec4.docx`（US）
- `reports/model-training/model-training-report-20260811T141458-e17c7c76.docx`（CN）

本轮闭环复核结果：

- 三市场最新验收任务与严格 OOF Agent Replay 均完成；Agent 只更新离线策略后验，仍受 No Trade、仓位、现金和 Paper-only 约束。
- ASX / US / CN 因子研究分别使用 36 只股票、44,447 / 44,460 / 44,448 行和约 1,235 个日期；每个市场的 Alpha 轻量进化均为 2/2 完成。
- 三市场均没有因子通过严格 outer-OOS 与 70 分准入门槛，因此正式因子权重仍为 0；失败候选和原因也进入轨迹，不再显示为“从未运行”。
- US 历史补齐批次处理 70 只股票，55 只达到至少 750 根；训练池中达到研究深度的股票由 112 提升至 158。仍不足 2,000 根的股票继续进入增量队列，不在单次任务中打穿接口额度。
- US 股票目录的 `ACT Symbol` 表头和非美交易所后缀已在解析、缓存读取和市场域三层过滤，避免污染训练池或浪费供应商请求。
- 三市场合并报告：`reports/model-training/model-training-report-20260811T142804-7c4b5035.docx`。
