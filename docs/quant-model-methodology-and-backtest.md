...
但如果有效样本少，62% 并不一定可靠。现在新增 `statisticalReliability`，使用样本权重 Wilson 区间计算：
...
targetHitLowerBound
stopRateUpperBound
directionHitLowerBound
effectiveIndependentSamples
statisticalReliabilityScore
```

如果统计下界不够强，历史因子会被扣分。这一步让“高置信”更接近统计意义上的可靠，而不是只看平均命中率。

## 27. 周期隔离与跨阶段模型稳定性

短期、中期和长期预测不是同一个学习任务。当前本地模型层把已到期的预测样本按策略持有期先分桶，再分别训练：

| 模型桶 | 预测周期 | 主要避免的问题 |
| --- | --- | --- |
| `short` | 1-7 个交易日 | 不把隔日/短线动量误用于 15 日以上趋势 |
| `mid` | 8-25 个交易日 | 让趋势、量能、因子和目标-止损标签在中周期内单独校准 |
| `long` | 26 个交易日以上 | 不让短线成交冲击主导中长期收益判断 |

每个桶至少需要 32 个已到期、point-in-time 的预测样本。样本不足时会显示 `collecting`，只能使用市场级模型作为保守回退；该回退不能提高高置信买入信号。

模型委员会另加了一道 expanding walk-forward 稳定性闸门。它不会只看一次 train/validation/test 切分，而是用多个后续时间段重复执行：

```text
fold 1: 用较早样本训练 -> 用下一段未见样本验证
fold 2: 扩展训练窗口 -> 用再下一段未见样本验证
fold 3: 再扩展训练窗口 -> 用最后一段未见样本验证
```

对技术 Ridge、因子 Ridge、订单流/Volume Profile Ridge、宽特征 Ridge、目标-止损 Meta 和序列状态代理分别统计：
...
因此，某个模型即使在最近一段行情里很准，只要跨阶段表现不稳定，就保持 `research_only`，不能用来抬高买入置信度。这个约束的目的不是提高表面命中率，而是压低“行情一变就改口”的错误高置信预测。
